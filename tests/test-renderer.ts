...
class FakeApi implements ViewerApi {
  ...
  unpair(): Promise<boolean> { return Promise.resolve(true); }
  state(): Promise<ViewerState> { return Promise.resolve(this.stateValue); }
  ...
  clipwatch(): Promise<ClipboardResult> { return Promise.resolve(emptyClipboard()); }
  quit(): Promise<void> { return Promise.resolve(); }
  onScan(listener: (scan: Scan) => void): void { this.scanListener = listener; }
  onStatus(listener: (status: ConnectionStatus) => void): void { this.statusListener = listener; }
  onClear(listener: () => void): void { this.clearListener = listener; }
  onRepair(listener: () => void): void { this.repairListener = listener; }
  onBump(listener: (bump: BumpEvent) => void): void { this.bumpListener = listener; }
  onBumpCleared(listener: (event: BumpClearedEvent) => void): void { this.bumpClearedListener = listener; }
  onClipWatch(listener: (result: ClipboardResult) => void): void { this.clipListener = listener; }
  onUnpaired(listener: () => void): void { this.unpairedListener = listener; }
  emitScan(scan: Scan): void { this.scanListener(scan); }
  emitStatus(status: ConnectionStatus): void { this.statusListener(status); }
  emitClear(): void { this.clearListener(); }
  emitRepair(): void { this.repairListener(); }
  emitBump(bump: BumpEvent): void { this.bumpListener(bump); }
  emitBumpCleared(event: BumpClearedEvent): void { this.bumpClearedListener(event); }
  emitClip(result: ClipboardResult): void { this.clipListener(result); }
  emitUnpaired(): void { this.unpairedListener(); }
}
...
class FakeRuntime implements RendererRuntime {
  ...
  timeouts: Array<() => void> = [];
  dateNow(): number { return this.wallNow; }
  monotonicNow(): number { return this.monoNow; }
  setInterval(callback: () => void): number { this.intervals.push(callback); return this.intervals.length; }
  clearInterval(): void { /* retained for deterministic inspection */ }
  setTimeout(callback: () => void): number { this.timeouts.push(callback); return this.timeouts.length; }
  clearTimeout(): void { /* retained for deterministic inspection */ }
}
...
function articleFor(hull: string): HTMLElement | null {
  return Array.from(document.querySelectorAll<HTMLElement>(".scan")).find((node) =>
    node.querySelector(".hull")?.textContent === hull) ?? null;
}
...
async function run(): Promise<void> {
  ...
  ok("only the matching timer becomes visible",
  ...
  ok("clearing uses the exact mapped timer", target?.querySelector<HTMLElement>(".bumprow")?.hidden === true);
  ok("server IDs never become selectors or DOM IDs",
  ...
  api.emitScan({
    id: "hostile-text", at: runtime.wallNow, scout: payload, hull: payload, system: payload,
    pilot: payload, scanGate: payload, headGate: payload, ammo: payload, sec: payload,
    prepped: payload, notes: payload, fitEft: payload, valueSell: 3_000_000_000,
    valueBuy: 2_000_000_000, valueSplit: 1_000_000_000, droppableSplit: 500_000_000,
    ehp: 900_000, fleetAll: [{ name: payload, ships: 12 }], cargoList: [{ name: payload, qty: 3 }],
  });
  const hostileArticle = articleFor(payload);
  const hostileOpen = hostileArticle?.querySelector<HTMLElement>(".scanOpen");
  hostileOpen?.click();
  const detail = document.getElementById("detail");
  ok("payload is readable in the scan and detail", hostileArticle?.textContent?.includes(payload) &&
    document.getElementById("detailBody")?.textContent?.includes(payload));
  ok("payload creates no foreign elements", !hostileArticle?.querySelector("img,script") && !detail?.querySelector("img,script"));
  ok("renderer has no HTML parsing sink", !source.includes("innerHTML") && source.includes("textContent") && source.includes("replaceChildren"));

  console.log("\n=== keyboard detail flow and focus containment ===");
  key(document, "Escape");
  hostileOpen?.focus();
  key(hostileOpen ?? document, "Enter");
  ok("Enter opens detail and focuses its close control",
  ...
  key(document, "Escape");
  ok("Escape closes detail and restores its scan row",
  ...
  api.emitClear();
  ok("clearing an open detail returns focus to the empty feed",
    detail?.getAttribute("aria-hidden") === "true" && document.activeElement?.id === "list");

  console.log("\n=== pairing focus and safe Escape behavior ===");
  api.emitRepair();
  const pair = document.getElementById("pair");
  ok("repair pairing is dismissible and initially focused",
    document.activeElement?.id === "server" && !(document.getElementById("pairCancel") as HTMLButtonElement).hidden);
  key(document.activeElement ?? document, "Tab", true);
  ...
  key(document, "Escape");
  ok("Escape closes safe pairing and restores re-pair",
    pair?.getAttribute("aria-hidden") === "true" && document.activeElement?.id === "repairBtn");
  api.emitUnpaired();
  key(document, "Escape");
  ok("mandatory pairing cannot be dismissed",
    pair?.getAttribute("aria-hidden") === null && (document.getElementById("pairCancel") as HTMLButtonElement).hidden);
  ...
  ok("clipboard state is exposed without timer announcement spam",
    clipboardMessage?.includes("sent fit") && status?.textContent === clipboardMessage);
  const clipButton = document.getElementById("clipBtn");
  ok("clipboard toggle exposes its state",
    clipButton?.getAttribute("aria-pressed") === "true" && clipButton.getAttribute("aria-label")?.startsWith("Disable"));
  ok("abbreviated controls have accessible names",
    ["quitBtn", "opBtn", "repairBtn", "clearBtn"].every((id) => document.getElementById(id)?.hasAttribute("aria-label")));
  ok("all global actions retain native keyboard semantics",
    ["clipBtn", "clearBtn", "opBtn", "repairBtn", "quitBtn"].every((id) =>
      document.getElementById(id)?.tagName === "BUTTON"));
  ...
  const foregrounds = ["text", "dim", "blue", "green", "amber", "red"].map((name) => rgb(cssValue(name)));
  const backdrops: Array<[number, number, number]> = [[0, 0, 0], [255, 255, 255]];
  const textPasses = alphas.length === 3 && alphas.every((alpha) => backdrops.every((backdrop) => {
    const background = [16, 17, 19].map((value, index) =>
      value * alpha + (backdrop[index] ?? 0) * (1 - alpha)) as [number, number, number];
    return foregrounds.every((foreground) => contrast(foreground, background) >= 4.5) &&
      contrast(rgb(cssValue("control-border")), background) >= 3 &&
      contrast(rgb(cssValue("focus")), background) >= 3;
  }));
  ok("normal text and controls meet WCAG thresholds over black and white", textPasses, alphas.join(","));
  ok("filled controls retain readable white labels",
    ["blue-fill", "green-fill"].every((name) => contrast([255, 255, 255], rgb(cssValue(name))) >= 4.5));
