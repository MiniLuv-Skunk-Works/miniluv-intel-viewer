...
class FakeClock {
  ...
  active(delay?: number): ClockEntry[] {
    return this.entries.filter((entry) => !entry.cleared && (delay === undefined || entry.delay === delay));
  }
...
class FakeRequest extends EventEmitter {
  ...
  end(): void { this.emit("socket", this.socket); }
...
function harness() {
  ...
  const request = (url: URL, options: http.RequestOptions, callback: (response: http.IncomingMessage) => void): http.ClientRequest => {
...
let oversized = false;
try { new SseParser(8).push("data: too much"); } catch (error) { oversized = error instanceof SseFrameTooLargeError; }
...
ok("feed authorization stays in the main process", transport.attempts[0]?.options.headers !== undefined &&
  (transport.attempts[0]?.options.headers as Record<string, string>).Authorization === "Bearer old-token");
...
ok("repeated end/error signals schedule one retry", clock.active(500).length === 1, clock.active().map((entry) => entry.delay));
...
ok("rapid re-pair replaces the request immediately", staleAttempt?.request.destroyed && transport.attempts.length === 4);
...
ok("stale callbacks cannot emit status or events", statuses.length === statusCount && !messages.includes("message:stale"));
ok("the replacement uses only the new dashboard and token", transport.attempts[3]?.url.origin === "https://new.example" &&
  (transport.attempts[3]?.options.headers as Record<string, string>).Authorization === "Bearer new-token");
...
const idleManager = new FeedConnectionManager({
  ...
  responseTimeoutMs: 20,
  idleTimeoutMs: 100,
  onStatus: (status) => idleStatuses.push(status.state + ("detail" in status ? ":" + status.detail : "")),
  onEvent: () => {},
  onUnauthorized: () => { unauthorized += 1; },
});
...
ok("heartbeat activity resets the idle deadline", originalIdle?.cleared && idleClock.active(100).length === 1);
const currentIdle = idleClock.active(100)[0];
if (currentIdle) idleClock.run(currentIdle);
ok("idle streams are closed and scheduled to reconnect", idleStatuses.some((status) => status.includes("idle timeout")) &&
  idleClock.active(1_000).length === 1 && idleResponse?.destroyed);
idleManager.stop();
...
const timeoutManager = new FeedConnectionManager({
  ...
  connectionTimeoutMs: 10,
  responseTimeoutMs: 20,
  onStatus: () => {}, onEvent: () => {}, onUnauthorized: () => {},
});
...
ok("the initial feed response has a deadline", timeoutClock.active().some((entry) => entry.delay >= 500));
timeoutManager.stop();
...
const contentManager = new FeedConnectionManager({
  ...
  onStatus: (status) => contentStatuses.push(status.state + ("detail" in status ? ":" + status.detail : "")),
...
ok("non-SSE feed content is rejected and retried", wrongContent?.destroyed &&
  contentStatuses.some((status) => status.includes("non-SSE")) && contentClock.active().length === 1);
contentManager.stop();
...
const authManager = new FeedConnectionManager({
  ...
  onStatus: () => {}, onEvent: () => {}, onUnauthorized: () => { unauthorized += 1; },
});
authManager.start({ serverUrl: "https://auth.example", token: "expired" });
authTransport.attempts[0]?.respond(401, "application/json");
ok("authentication expiry is terminal and does not retry", unauthorized === 1 && authClock.active().length === 0);
