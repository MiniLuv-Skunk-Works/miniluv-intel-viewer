...
class MemoryFiles implements CredentialFileSystem {
  ...
  async mkdir(): Promise<void> { this.events.push("mkdir"); }
...
void (async () => {
  ...
  ok("empty profile starts unpaired", (await store.initialize(undefined)).status === "empty" && store.get() === null);
  ok("new token is encrypted and retained in memory", await store.set("new-secret") && store.get() === "new-secret");
  ok("credential file does not contain plaintext", files.files.get(file)?.includes(Buffer.from("new-secret")) === false);

  const restarted = new CredentialStore(file, crypto, files);
  ok("restart decrypts the stored credential", (await restarted.initialize(undefined)).status === "ready" && restarted.get() === "new-secret");
  ok("unpair clears memory and disk", await restarted.clear() && restarted.get() === null && !files.files.has(file));
  ...
  ok("migration writes before reporting completion", migrationFiles.events.includes("write") && migrationStore.get() === "legacy-secret");
  ...
  ok("unavailable encryption fails closed", unavailableResult.status === "unavailable" && unavailable.get() === null);
  ...
  ok("corrupt ciphertext requests re-pairing", (await corrupt.initialize(undefined)).status === "corrupt" && corrupt.get() === null);
  ...
  ok("key rotation rewrites the credential", (await rotated.initialize(undefined)).status === "ready" &&
    rotateCrypto.encryptions === encryptionsBeforeRestart + 1 && rotated.get() === "rotate-me");

  ok("oversized replacement tokens are rejected transactionally",
    !await rotated.set("x".repeat(8_193)) && rotated.get() === "rotate-me");

  console.log("\n" + (fail === 0 ? "ALL " + pass + " PASSED" : pass + " passed, " + fail + " FAILED"));
  process.exit(fail === 0 ? 0 : 1);
})().catch((error: unknown) => {
  console.error("credential test failed", error instanceof Error ? error.message : "unknown error");
  process.exit(1);
});
